...
func cmdAdd(args *skel.CmdArgs) error {
	...
	conf := NetConf{}
	fmt.Fprintf(os.Stderr, "******Calico CNI args is: %s\n", args)
	fmt.Fprintf(os.Stderr, "******Calico CNI args.StdinData is: %s\n", args.StdinData)
	...
	fmt.Fprintf(os.Stderr, "******Calico CNI workload is: %s\n", workload)
	...
	fmt.Fprintf(os.Stderr, "******Calico CNI hostname is: %s\n", hostname)
	...
	fmt.Fprintf(os.Stderr, "******Calico CNI endpoints is: %s\n", endpoints)
	...
	/*
	profileID := conf.Name
	fmt.Fprintf(os.Stderr, "******Calico CNI profileID is: %s\n", profileID)

	fmt.Fprintf(os.Stderr, "******Calico CNI paths is: %s\n", os.Getenv("CNI_PATH"))
	fmt.Fprintf(os.Stderr, "******Calico CNI type is: %s\n", conf.IPAM.Type)
	logger.WithFields(log.Fields{"paths": os.Getenv("CNI_PATH"),
		"type": conf.IPAM.Type}).Debug("Looking for IPAM plugin in paths")
	fmt.Fprintf(os.Stderr, "******Calico CNI args.StdinData is: %s\n", args.StdinData)
	//conf.IPAM.Type = "calico.bak"
	...
	*/
	...
	// function.
	fmt.Fprintf(os.Stderr, "******Calico CNI orchestrator is **: %s\n", orchestrator)
	if orchestrator == "k8s" {
		//conf.IPAM.Type = "calico-ipam"
		/*ipam.ExecAdd(conf.IPAM.Type, args.StdinData)

		fmt.Fprintf(os.Stderr, "******Calico CNI args.Path= %s\n", args.Path)
		...
		if args.StdinData,err = json.Marshal(&conf); err != nil {
			return fmt.Errorf("failed to load netconf: %v", err)
		}*/

		fmt.Fprintf(os.Stderr, "******Calico CNI args.ContainerID= %s\n", args.ContainerID)
		fmt.Fprintf(os.Stderr, "******Calico CNI args.Path= %s\n", args.Path)
		fmt.Fprintf(os.Stderr, "******Calico CNI args.Netns= %s\n", args.Netns)
		fmt.Fprintf(os.Stderr, "******Calico CNI args.Args= %s\n", args.Args)
		fmt.Fprintf(os.Stderr, "******Calico CNI args= %s\n", args)


		if result, err = k8s.CmdAddK8s(args, conf, hostname, calicoClient, endpoint); err != nil {
			fmt.Fprintf(os.Stderr, "******Calico CNI result err = %s\n", err)
			return err
		}
		...
		c := exec.Cmd{
			Env: env,
			Path:   args.Path,
			Args:  []string{args.Path},
			Stdin:  bytes.NewBuffer(args.StdinData),
			Stdout: stdout,
			//Stderr: e.Stderr,
		}
		if err := c.Run(); err != nil {
			fmt.Fprintf(os.Stderr, "******Calico CNI error=%s\n", err)
		}
		...
		if endpoint != nil {
			...
			logger.WithFields(log.Fields{"paths": os.Getenv("CNI_PATH"),
				"type": conf.IPAM.Type}).Debug("Looking for IPAM plugin in paths")
			fmt.Fprintf(os.Stderr, "******Calico CNI args.StdinData is: %s\n", args.StdinData)
			fmt.Fprintf(os.Stderr, "******Calico CNI result is: %s\n", result)
			result, err = ipam.ExecAdd(conf.IPAM.Type, args.StdinData)
			logger.WithField("result", result).Info("Got result from IPAM plugin")
			if err != nil {
				return err
