...
func GetKubeClient() (*kubernetes.Clientset, error) {
	glog.Info(" validateNetworkParas masterURL, kubeconfig ", masterURL, kubeconfig)
...
func validateNetworkParas(logicalNetwork *genieUtils.LogicalNetwork ) *v1beta1.AdmissionResponse {
	...
	glog.Info(" validateNetworkParas phyNwName = %s", phyNwName)
	if ("" == phyNwName){
	...
	glog.Info(" validateNetworkParas client, error ", client, error)
	...
	glog.Info(" validateNetworkParas physicalNwPath  ", physicalNwPath)
	...
	if err != nil {
		...
		glog.Info(" validateNetworkParas failed %s, err %s  ", ERR_PHYSICAL_NW_NOT_FOUND, err)
		return &admissionResponse
	}
	...
	if err = json.Unmarshal(physicalNwObj, &physicalNwInfo); err != nil {
		...
		glog.Info(" validateNetworkParas failed %s, err %s  ", ERR_INCORRECT_PHYSICALNETWORK_PARAS, err)
		return &admissionResponse
	}
	...
	if physicalNwInfo.Spec.SharedStatus.DedicatedStatus == true {
		...
		if ("" == selectedPluginName){
			...
			glog.Info(" validateNetworkParas ERR_NO_PLUGIN_MENTIONED ", ERR_NO_PLUGIN_MENTIONED_IN_PHYSICAL_NETWORK)
			return &admissionResponse
		...
		glog.Info(" validateNetworkParas checkIfValidInnerSubnet ", outerSubnet, logicalNetwork.Spec.SubSubnet, isvalidSubnet)
		...
		glog.Info(" validateNetworkParas ERR_NO_PLUGIN_MENTIONED ", ERR_NO_PLUGIN_MENTIONED_IN_LOGICAL_NETWORK)
		return &admissionResponse
		...
		glog.Info(" validateNetworkParas ERR_NO_PLUGIN_MENTIONED ", ERR_SUBNET_NOT_SPECIFIED)
		return &admissionResponse
	...
	if (nil != PluginSubnetUsageDataList){
		for i := range PluginSubnetUsageDataList {
			if (PluginSubnetUsageDataList[i].PluginName == selectedPluginName){
				...
				glog.Info(" validateNetworkParas PluginSubnetUsageDataList isPluginFound found ",  selectedPluginName)
                                break
			}
	...
	if (false == isPluginFound){
		...
		glog.Info(" validateNetworkParas entry newly added, PluginSubnetUsageDataList  ",  PluginSubnetUsageDataList)
	}else {
		...
		if ("" != pluginSubnetUsageData.SubnetUsageData[selectedSubnet]){
			...
			glog.Info(" validateNetworkParas exact subent already found  ", selectedSubnet)
			return &admissionResponse
		}
		for key, _ := range pluginSubnetUsageData.SubnetUsageData {
			if(checkSubnetOverlap(selectedSubnet, key)){
				admissionResponse.Result = &metav1.Status{
					Reason: ERR_SUBNET_OVERLAP_WITH_OTHER,
				}
				glog.Info(" validateNetworkParas similar subnet already found  ", selectedSubnet)
				return &admissionResponse
			}
		}

		pluginSubnetUsageData.SubnetUsageData[selectedSubnet] = logicalNetwork.ObjectMeta.Name
		glog.Info(" validateNetworkParas new subnet added  ", PluginSubnetUsageDataList)
	}
